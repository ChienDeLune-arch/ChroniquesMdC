...
console.log('Error:', JSON.stringify(error))
  ...
  if (q) {
    query = query.textSearch('title, excerpt', q, { type: 'websearch', config: 'french' })
  }

  const { data: posts, count } = await query.range(offset, offset + PER_PAGE - 1)
  const totalPages = Math.ceil((count ?? 0) / PER_PAGE)

  // Post à la une (featured, premier de la liste si pas de filtre)
    ...
    <div className="max-w-7xl mx-auto px-4 py-10">
      ...
      {totalPages > 1 && (
        <div className="flex justify-center items-center gap-2 mt-12">
          {page > 1 && (
            <a
              href={buildUrl({ tag, q, page: page - 1 })}

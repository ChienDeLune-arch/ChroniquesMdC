...
export function Comments({ postId, userId, userProfile }: CommentsProps) {
      ...
      .on(
        ...
        async payload => {
          ...
          const { data: profile } = await supabase
            ...
            .single()
          const comment = { ...payload.new, author: profile, replies: [] } as unknown as CommentWithReplies
          ...
          setComments(prev => {
            ...
            // Éviter les doublons (optimistic + realtime)
  ...
  async function load() {
    ...
    if (data) {
      // Construire l'arbre (1 niveau de réponses)
  ...
  if (!userId) {
    return (
      <div className="p-4 surface-card text-center text-sm text-secondary">
        <a href="/auth/login" className="text-[rgb(var(--color-accent))] underline">
          Connecte-toi
        </a>{' '}
